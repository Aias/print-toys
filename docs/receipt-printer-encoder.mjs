...
class LanguageStarPrnt {
  ...
  image(image, width, height, mode) {
    ...
    const getPixel = (x, y) =>
      typeof image.data[(width * y + x) * 4] === "undefined" ||
      image.data[(width * y + x) * 4] > 0
        ? 0
        : 1;
      ...
      for (const chunk of chunks) {
        ...
        if (length + chunk.length * width > columns) {
          ...
          if (chunk.length * width > columns) {
            ...
            while (
              (piece = letters.splice(0, Math.floor(columns / width))).length
            ) {
...
class LineComposer {
  ...
  text(value, codepage) {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].length) {
        ...
        this.add(
          { type: "text", value: lines[i], codepage },
  ...
  fetch(options) {
    ...
    const buffer = this.#buffer.filter(
      (item) => item.type !== "align" || item.payload
    );
    ...
    if (this.#cursor === 0 && (options.ignoreAlignment || !this.#embedded)) {
      result = this.#merge([...this.#stored, ...buffer, ...store]);
    } else {
      if (this.#align === "right") {
        let last;
        ...
        if (typeof last === "number") {
          ...
          if (
            buffer[last].type === "text" &&
            buffer[last].value.endsWith(" ")
          ) {
...
const printerDefinitions = {
  ...
  "hp-a779": {
    ...
    capabilities: {
      ...
      pdf417: {
        supported: false,
        fallback: { type: "barcode", symbology: 75 },
      },
      cutter: { feed: 4 },
    },
  },
...
class ReceiptPrinterEncoder {
  ...
  constructor(options) {
    ...
    if (typeof options.language === "string") {
      ...
      defaults.codepageMapping =
        options.language === "esc-pos" ? "epson" : "star";
    }
    ...
    if (typeof options.printerModel === "string") {
      ...
      this.#printerCapabilities =
        printerDefinitions[options.printerModel].capabilities;
      ...
      defaults.feedBeforeCut =
        this.#printerCapabilities?.cutter?.feed || defaults.feedBeforeCut;
      defaults.imageMode =
        this.#printerCapabilities?.images?.mode || defaults.imageMode;
    }
    ...
    if (this.#options.language === "esc-pos") {
      ...
      this.#options.language === "star-prnt" ||
      this.#options.language === "star-line"
    ) {
    ...
    */
    ...
    if (typeof this.#options.autoFlush === "undefined") {
      this.#options.autoFlush =
        !this.#options.embedded && this.#options.language == "star-prnt";
    }

    /* Check column width */

    if (
      ![32, 35, 42, 44, 48].includes(this.#options.columns) &&
    ...
    if (typeof this.#options.codepageMapping === "string") {
      if (
        typeof codepageMappings[this.#options.language][
          this.#options.codepageMapping
        ] === "undefined"
      ) {
  ...
  #reset() {
    this.#queue = [];
    this.#codepage =
      this.#options.language == "esc-pos" ? "cp437" : "star/standard";
  ...
  font(value) {
    if (this.#options.embedded) {
      throw new Error(
        "Changing fonts is not supported in table cells or boxes"
      );
    }

    if (this.#composer.cursor > 0) {
      throw new Error(
        "Changing fonts is not supported in the middle of a line"
      );
    }
    ...
    if (matches) {
      value = Object.entries(this.#printerCapabilities.fonts).find(
    ...
    if (typeof this.#printerCapabilities.fonts[value] === "undefined") {
      return this.#error(
        "This font is not supported by this printer",
        "relaxed"
      );
    }
    ...
    for (let r = 0; r < data.length; r++) {
      ...
      for (let c = 0; c < columns.length; c++) {
        ...
        for (let p = lines[c].length; p < maxLines; p++) {
          ...
          const line = {
            commands: [{ type: "space", size: columns[c].width }],
            height: 1,
          };
  ...
  rule(options) {
    ...
    this.#composer.text(
      (options.style === "double" ? "═" : "─").repeat(options.width),
      "cp437"
    );
    this.#composer.flush({ forceNewline: true });
  ...
  barcode(value, symbology, height) {
    ...
    if (this.#printerCapabilities.barcodes.supported === false) {
      return this.#error(
        "Barcodes are not supported by this printer",
        "relaxed"
      );
    }
    ...
    if (
      typeof symbology === "string" &&
      !this.#printerCapabilities.barcodes.symbologies.includes(symbology)
    ) {
      return this.#error(
        `Symbology '${symbology}' not supported by this printer`,
        "relaxed"
      );
    }
  ...
  qrcode(value, model, size, errorlevel) {
    ...
    if (this.#printerCapabilities.qrcode.supported === false) {
      return this.#error(
        "QR codes are not supported by this printer",
        "relaxed"
      );
    }
    ...
    if (
      options.model &&
      !this.#printerCapabilities.qrcode.models.includes(String(options.model))
    ) {
      return this.#error(
        "QR code model is not supported by this printer",
        "relaxed"
      );
    }
  ...
  pdf417(value, options) {
    ...
    if (this.#printerCapabilities.pdf417.supported === false) {
      ...
      if (typeof this.#printerCapabilities.pdf417.fallback === "object") {
        return this.barcode(
          value,
          this.#printerCapabilities.pdf417.fallback.symbology
        );
      }

      return this.#error(
        "PDF417 codes are not supported by this printer",
        "relaxed"
      );
    }
  ...
  image(input, width, height, algorithm, threshold) {
    ...
    name == "Canvas" && typeof input.getContext !== "undefined"
      ? (type = "node-canvas")
      : null;
    name == "Image" ? (type = "node-canvas-image") : null;
    name == "Image" && typeof input.frames !== "undefined"
      ? (type = "node-read-image")
      : null;
    name == "Object" && input.data && input.info ? (type = "node-sharp") : null;
    name == "View3duint8" && input.data && input.shape
      ? (type = "ndarray")
      : null;
    name == "Object" && input.data && input.width && input.height
      ? (type = "object")
      : null;
   ...
   */
    ...
    if (codepage === null) {
      ...
      return [{ type: "text", payload: [...fragment] }];
    }
    ...
    for (const fragment of fragments) {
      ...
      buffer.push(
        {
          type: "codepage",
          payload: this.#language.codepage(
            this.#codepageMapping[fragment.codepage]
          ),
        },
