...
function parseBody(body: unknown): PrintCommand[] | null {
  ...
  // Array of commands
  if (Array.isArray(body) && body.length > 0 && body.every(isPrintCommand)) return body;
  return null;
}
...
async function processCommands(commands: PrintCommand[]): Promise<Uint8Array> {
  ...
  for (const cmd of commands) {
    if (isMarkdownCommand(cmd)) {
      ...
      const html = await markdown.parse(convertUrlsToImageMarkdown(cmd.content));
...
export async function POST(request: Request) {
  ...
  try {
    ...
    fireAndForget(
      ...
      (error) => console.error(`[API print failed for job ${job.jobId}]:`, error)
    );
